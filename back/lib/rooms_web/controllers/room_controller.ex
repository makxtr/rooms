defmodule RoomsWeb.RoomController do
  ...
  def show(conn, %{"hash" => hash}) do
    case Rooms.fetch_room_by_hash(hash) do
      {:ok, room} ->
        json(conn, %{
          room: Rooms.format_room_response(room)
        })
  ...
  def create(conn, params) do
    ...
    case Rooms.create_room(params_with_creator) do
      {:ok, room} ->
        conn
        |> put_status(:created)
        |> json(Rooms.format_room_response(room))  # Возвращаем напрямую для фронта
  ...
  def enter(conn, %{"hash" => hash} = params) do
    ...
    case Rooms.find_or_create_room(hash, attrs) do
      {:ok, room} ->
    ...
    case Rooms.fetch_room_by_hash(hash) do
      {:ok, room} ->
        case Rooms.update_room(room, params) do
          {:ok, updated_room} ->
            json(conn, Rooms.format_room_response(updated_room))
  ...
  def search(conn, _params) do
    case Rooms.get_random_room() do
      nil ->
        # Если нет комнат, создаем дефолтную
        case Rooms.find_or_create_room("general", %{"topic" => "Общий чат"}) do
          {:ok, room} ->
            json(conn, Rooms.format_room_response(room))  # Возвращаем напрямую для фронта
