...
(function () {
    ...
    function showAlarm(room, on) {
        ...
        if (on) {
            alarmOn
                .find(".alarm-time")
                .text(getHumanTime(room.data.min_session_created));
        }
    }
...
(function () {
    ...
    function showIgnored(role) {
        ...
        termValue.text(
            term ? terms[term] : tr ? date.toHumanAgo() : "пожизненно",
        );
        termValue.attr(
            "title",
            String.mix(
                "Начало игнора $1 в $2",
                date.toSmartDate().toLowerCase(),
                date.toHumanTime(),
            ),
        );
    ...
    function selectTerm(term) {
        ...
        termOptions
            .eq(term ? termsIndex[term] : 3)
            .addClass("moder-term-selected");
    }
...
(function () {
    ...
    Profile.on("show", function (role, isMy) {
        targetMessage = Profile.context.inTalk
            ? getMessage(Profile.context.target)
            : null;
    });
